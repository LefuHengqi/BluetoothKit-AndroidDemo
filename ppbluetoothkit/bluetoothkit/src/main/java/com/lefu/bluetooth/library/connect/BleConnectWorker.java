...
import com.lefu.bluetooth.library.connect.listener.GattResponseListener;
import com.lefu.bluetooth.library.connect.listener.IBluetoothGattResponse;
import com.lefu.bluetooth.library.connect.listener.ReadCharacterListener;
import com.lefu.bluetooth.library.connect.listener.ReadDescriptorListener;
import com.lefu.bluetooth.library.connect.listener.ReadRssiListener;
import com.lefu.bluetooth.library.connect.listener.RequestMtuListener;
import com.lefu.bluetooth.library.connect.listener.ServiceDiscoverListener;
import com.lefu.bluetooth.library.connect.listener.WriteCharacterListener;
import com.lefu.bluetooth.library.connect.listener.WriteDescriptorListener;
import com.lefu.bluetooth.library.connect.response.BluetoothGattResponse;
...
import com.lefu.bluetooth.library.utils.Version;
...
public class BleConnectWorker implements Handler.Callback, IBleConnectWorker, IBluetoothGattResponse, ProxyInterceptor, RuntimeChecker {
    ...
    public void onCharacteristicWrite(BluetoothGattCharacteristic characteristic, int status, byte[] value) {
        BluetoothLog.v("onCharacteristicWrite for " + mBluetoothDevice.getAddress() +
                ": status = " + status +
                ", service = " + characteristic.getService().getUuid() +
                ", character = " + characteristic.getUuid() +
...
//                ByteUtils.byteToString(value)));
    ...
    public void closeGatt() {
        ...
        if (mGattResponseListener != null) {
            if (mGattResponseListener instanceof ServiceDiscoverListener) {
                if (((ServiceDiscoverListener) mGattResponseListener).isRetry()) {
                    //返回fasle则通知外部断开连接，如返回true内部会自动重试，不对外回调断开连接状态
                    BluetoothLog.v(String.format("closeGatt for %s 内部重连，不回调外部，", getAddress()));
                    return;
                }
            }
        }
...
//                mBluetoothDevice.getAddress(), service, character, ByteUtils.byteToString(value)));
        BluetoothLog.v("writeCharacteristicWithNoRsp for " + mBluetoothDevice.getAddress() +
                ": service = " + service +
                ", character = " + character +
                ", value = " + value.length);
        checkRuntime();
...
//        }
        ...
        BluetoothLog.v("开始写系统Api mBluetoothGatt.writeCharacteristic");
        if (!mBluetoothGatt.writeCharacteristic(characteristic)) {
            BluetoothLog.e("writeCharacteristic failed");
            return false;
        }
        BluetoothLog.v("写系统Api成功 mBluetoothGatt.writeCharacteristic");
        return true;
    }

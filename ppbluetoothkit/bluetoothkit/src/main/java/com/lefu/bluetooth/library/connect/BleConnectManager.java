...
public class BleConnectManager {
    ...
    public static void writeNoRspDirectly(String mac, UUID service, UUID character, byte[] value, BleGeneralResponse response) {
        BluetoothLog.d("BleConnectManager writeNoRspDirectly");
        IBleConnectMaster bleConnectMaster = getBleConnectMaster(mac);
        bleConnectMaster.writeNoRspDirectly(service, character, value, response);
    }

...
import com.lefu.bluetooth.library.connect.IBleConnectWorker;
...
public abstract class BleRequest implements IBleConnectWorker, IBleRequest, Handler.Callback, GattResponseListener, RuntimeChecker {
    ...
    public void onResponse(final int code) {
        ...
        BluetoothLog.i("onResponse code = " + code + " mFinished = " + mFinished);
        if (mFinished) {
            return;
        } else {
            mFinished = true;
        }
        if (this instanceof BleWriteNoRspDirectRequest) {
            if (mResponse != null) {
                BluetoothLog.d("BleWriteNoRspDirectRequest BleGeneralResponse onResponse mResponse = " + mResponse);
            ...
            mResponseHandler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        if (mResponse != null) {
                            BluetoothLog.d("onResponse mResponse = " + mResponse);
                            mResponse.onResponse(code, mExtra);
                        }
                    } catch (Throwable e) {
                        e.printStackTrace();

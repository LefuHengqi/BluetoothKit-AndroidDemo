...
class PeripheralDorreActivity : AppCompatActivity() {
    ...
    var controller: PPBlutoothPeripheralDorreController? = PPBlutoothPeripheralDorreInstance.instance.controller
    ...
    val bleStateInterface = object : PPBleStateInterface() {
        ...
        override fun monitorBluetoothSwitchState(ppBleSwitchState: PPBleSwitchState?) {
            if (ppBleSwitchState == PPBleSwitchState.PPBleSwitchStateOff) {
                ...
                Toast.makeText(this@PeripheralDorreActivity, getString(R.string.system_bluetooth_disconnect), Toast.LENGTH_SHORT).show()
            } else if (ppBleSwitchState == PPBleSwitchState.PPBleSwitchStateOn) {
                addPrint(getString(R.string.system_blutooth_on))
                Toast.makeText(this@PeripheralDorreActivity, getString(R.string.system_blutooth_on), Toast.LENGTH_SHORT).show()
            }
        }
                ...
                MsgDialog.init(supportFragmentManager)
                    ...
                    .setPositiveButton(getString(R.string.confirm), View.OnClickListener() {
                        ...
                        if (deviceModel.deviceCalcuteType == PPScaleDefine.PPDeviceCalcuteType.PPDeviceCalcuteTypeAlternate8_0
                            ...
                            //4电极交流算法  24项数据
                            val intent = Intent(this@PeripheralDorreActivity, Calculate4ACActivitiy::class.java)
                            intent.putExtra("bodyDataModel", "bodyDataModel")
                            startActivity(intent)
                        }
                    })
    ...
    val modeChangeInterface = object : PPTorreDeviceModeChangeInterface {
        ...
        override fun demoModeSwitchCallBack(type: Int, state: Int) {
            if (type == 1) {
                if (state == 0) {
                    addPrint("设置成功")
                    Toast.makeText(this@PeripheralDorreActivity, "设置成功", Toast.LENGTH_SHORT).show()
                } else {
                    addPrint("设置失败")
...
//            PPBlutoothKit.setDebug(true)
    ...
    val deviceLogInterface = object : PPDeviceLogInterface {
        ...
        override fun syncLogEnd(logFilePath: String?) {
            ...
            Toast.makeText(this@PeripheralDorreActivity, "Sync Log Success", Toast.LENGTH_SHORT).show()
            FileUtil.sendEmail(this@PeripheralDorreActivity, logFilePath)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 2 && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            if (Environment.isExternalStorageManager()) {
                ...
                Toast.makeText(this@PeripheralDorreActivity, "存储权限获取失败", Toast.LENGTH_SHORT).show()
            }

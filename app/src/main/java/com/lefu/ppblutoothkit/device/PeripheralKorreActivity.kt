...
import com.lefu.ppblutoothkit.device.instance.PPBlutoothPeripheralKorreInstance
 ...
 * 设备类型 人体秤
 */
class PeripheralKorreActivity : AppCompatActivity() {
    ...
    var controller: PPBlutoothPeripheralKorreController? = PPBlutoothPeripheralKorreInstance.instance.controller
        ...
        MsgDialog.init(supportFragmentManager)
            ...
            .setPositiveButton(getString(R.string.confirm), View.OnClickListener() {
                if (deviceModel.deviceCalcuteType == PPScaleDefine.PPDeviceCalcuteType.PPDeviceCalcuteTypeAlternate8_0
                    ...
                    //8电极交流算法  48项数据
                    val intent = Intent(this@PeripheralKorreActivity, Calculate8Activitiy::class.java)
                    ...
                    //4电极交流算法  24项数据
                    val intent = Intent(this@PeripheralKorreActivity, Calculate4AC2ChannelActivitiy::class.java)
                    intent.putExtra("bodyDataModel", "bodyDataModel")
                    startActivity(intent)
                } else {
                    //4电极交流算法  24项数据
                    val intent = Intent(this@PeripheralKorreActivity, Calculate4ACActivitiy::class.java)
                    intent.putExtra("bodyDataModel", "bodyDataModel")
                    startActivity(intent)
                }
            })
    ...
    val modeChangeInterface = object : PPTorreDeviceModeChangeInterface {
        ...
        override fun demoModeSwitchCallBack(type: Int, state: Int) {
            if (type == 1) {
                if (state == 0) {
                    addPrint("设置成功")
                    Toast.makeText(this@PeripheralKorreActivity, "设置成功", Toast.LENGTH_SHORT).show()
                } else {
                    addPrint("设置失败")
                    Toast.makeText(this@PeripheralKorreActivity, "设置失败", Toast.LENGTH_SHORT).show()
                }
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 2 && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            if (Environment.isExternalStorageManager()) {
                ...
                Toast.makeText(this@PeripheralKorreActivity, "存储权限获取失败", Toast.LENGTH_SHORT).show()
            }

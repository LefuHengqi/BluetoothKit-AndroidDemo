...
object PPBluetoothPeripheralKorreInstance {
    ...
    fun syncTime(callback: (success: Boolean) -> Unit) {
        addPrint("syncTime")
        controller?.getTorreDeviceManager()?.syncTimeKorre(callback)
    }

    fun syncUnit(unitType: PPUnitType, resultCallBack: PPBleSendResultCallBack) {
        addPrint("syncUnit unitType:${unitType}")
        controller?.getTorreDeviceManager()?.syncUnitKorre(unitType, resultCallBack)
    }
    ...
    fun addPrint(msg: String) {
        if (msg.isNotEmpty()) {
            Logger.d("${PPBluetoothPeripheralKorreInstance::class.java.simpleName}:msg:Korre $msg")
        }
    }

...
object PPBluetoothPeripheralLorreInstance {

    var controller: PPBluetoothPeripheralLorreController? = null

    var currentNum = 0 //记录下发用户信息的个数  若currentNum为-1, 则表示只下发指定用户数据

    init {
        if (controller == null) {
            controller = PPBluetoothPeripheralLorreController()
        }
    }
    ...
    fun startFoodManager(sendResultCallBack: PPBleSendResultCallBack? = null) {
        controller?.getTorreDeviceManager()?.startFoodManager(sendResultCallBack)
    }
    ...
    fun endFoodManager(sendResultCallBack: PPBleSendResultCallBack? = null) {
        controller?.getTorreDeviceManager()?.endFoodManager(sendResultCallBack)
    }
    ...
    fun syncTime(callback: (success: Boolean) -> Unit) {
        addPrint("syncTime")
        controller?.getTorreDeviceManager()?.syncTimeLorre(callback)
    }

    fun syncUnit(unitType: PPUnitType, resultCallBack: PPBleSendResultCallBack) {
        addPrint("syncUnit unitType:${unitType}")
        controller?.getTorreDeviceManager()?.syncUnitLorre(unitType, resultCallBack)
    }
